...
function createMeetupEvents(data) {
  ...
  for(let i =0; i< data.length; i++){
    ...
    cardText.innerHTML = 'Date: ' + eventItem['local_date'] + '<br/>' + 'Time: ' + eventItem['local_time'];
    cardBody.appendChild(cardText);

    //Create location text
    let cardTextLocation = document.createElement('p');
    cardTextLocation.setAttribute('class', 'card-text');
    cardTextLocation.innerHTML = 'Location: ' + eventItem['venue']['name'] + '<br/>' + eventItem['venue']['address_1'];
